...
import { AlertTriangle, Dna, Download, KeyRound, Loader2 } from "lucide-react";
...
const getCredentialsFromConnection = (
...
function SchemaPage() {
	...
	const seedMutation = useMutation({
		...
		onSuccess: async (result, variables) => {
			toast.success(
				`Injected ${result.inserted} rows into ${variables.tableName}.`,
			);
			await queryClient.invalidateQueries({
				queryKey: ["schema", activeConnection?.id],
			});
		...
		<main className="mx-auto flex min-h-screen w-full flex-col gap-8 p-6 md:p-10 font-mono">
				...
				<div>
					...
					<div className="mt-2 flex items-center gap-3">
						...
						<div className="bg-black border-2 border-orange-500 px-3 py-1 text-orange-500 font-black text-xl tracking-widest flex items-center gap-2">
							<span className="text-[10px] uppercase text-orange-700">
								TABLES:
							</span>
					...
					<div className="flex items-center gap-2 bg-zinc-100 p-2 border-2 border-black">
						...
						>
							...
							<SelectContent className="rounded-none border-2 border-black shadow-hardware font-mono">
								<SelectItem
									value="json"
									className="font-bold uppercase rounded-none focus:bg-zinc-200 hover:!bg-zinc-400 bg-white text-black cursor-pointer"
								>
									Export JSON
								</SelectItem>
								<SelectItem
					...
					<AlertDialog>
						...
						<AlertDialogContent className="rounded-none border-4 border-red-600 shadow-hardware font-mono p-6">
							...
							<AlertDialogFooter className="mt-6">
								<AlertDialogCancel className="rounded-none border-2 border-black shadow-hardware active:translate-x-[2px] active:translate-y-[2px] active:shadow-none font-bold uppercase">
									Cancel
								</AlertDialogCancel>
								<AlertDialogAction
									onClick={() => {
				...
				<DialogContent className="rounded-none border-4 border-black shadow-hardware font-mono sm:max-w-xl md:max-w-3xl">
					<DialogHeader>
						<DialogTitle className="text-2xl font-black uppercase text-foreground">
							Restore local SQL dump
						</DialogTitle>
						<DialogDescription className="text-white font-bold">
							Select a dump from the exports directory to restore it into the
							active database.
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4 my-4">
						...
						{availableDumps.length > 0 ? (
							<div className="border-2 border-black p-2 bg-zinc-100 shadow-inner">
								<Select
									value={selectedDumpPath}
									onValueChange={setSelectedDumpPath}
								>
									...
									<SelectContent className="rounded-none border-2 border-black shadow-hardware font-mono">
										{availableDumps.map((filePath) => (
											<SelectItem
												key={filePath}
												value={filePath}
												className="rounded-none cursor-pointer  hover:!bg-zinc-400 bg-white text-black focus:bg-zinc-200"
											>
												{filePath}
											</SelectItem>
										))}
					...
					<DialogFooter>
						<DialogClose asChild>
							<Button
								type="button"
								disabled={isRestoringDump}
								className="rounded-none border-2 border-black shadow-hardware active:translate-x-[2px] active:translate-y-[2px] active:shadow-none font-bold uppercase bg-zinc-200 text-black hover:bg-zinc-300"
							>
								Cancel
							</Button>
						</DialogClose>
			...
			{schemaQuery.isLoading ? (
				<div className="flex items-center gap-3 bg-zinc-950 text-white p-4 border-2 border-black shadow-hardware">
					...
					<p className="text-xl font-black uppercase tracking-widest text-orange-500 blink">
			...
			{isSeeding ? (
				<div className="flex items-center gap-3 bg-yellow-400 text-black p-4 border-2 border-black shadow-hardware">
					<Loader2 className="animate-spin w-6 h-6" />
					<p className="text-xl font-black uppercase tracking-widest">
						Injecting noise into {seedingTableName}...
					</p>
				</div>
			) : null}
			...
			{!schemaQuery.isLoading && !schemaError ? (
				<section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
					{tables.map((table) => (
						<Card
							key={table.tableName}
							className="rounded-none border-2 border-black dark:border-white p-0 bg-zinc-50 dark:bg-zinc-950 shadow-hardware dark:shadow-hardware-dark flex flex-col h-full"
						>
							<CardHeader className="flex-row items-center justify-between gap-3 space-y-0 bg-black text-white p-4 border-b-2 border-black dark:border-white">
								<CardTitle className="text-lg font-black uppercase tracking-widest truncate">
									{table.tableName}
								</CardTitle>
								<div className="flex items-center gap-2">
									...
									<AlertDialog>
										<AlertDialogTrigger asChild>
											...
											>
												{clearingTableName === table.tableName
													? "WAIT"
													: "CLEAR"}
											</Button>
										</AlertDialogTrigger>
